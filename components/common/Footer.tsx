...
export function Footer(): JSX.Element {
  const metadata = defaultSEOConfig;
      ...
      <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:py-16 lg:px-8">
        <div className="xl:grid xl:grid-cols-3 xl:gap-8">
          <div className="xl:col-span-1">
            <Image src="/icon.png" alt="echo karriere logo" width={32} height={32} />
          ...
          <div className="grid grid-cols-3 gap-8 xl:col-span-2">
            {navigation.map((item) => (
              <div key={item.title}>
                <h4 className="text-sm font-semibold tracking-wider text-gray-600 uppercase">{item.title}</h4>
                <ul className="mt-4">
                  {item.items.map((child, index) => (
                    <li key={child.to} className={index !== 0 ? "mt-4" : ""}>
                      <Link href={child.to}>
                        <a href={child.to} className="text-base text-gray-500 hover:text-gray-900">
                          {child.title}
                        </a>
                      </Link>
                    </li>
